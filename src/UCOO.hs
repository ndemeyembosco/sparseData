...
instance (U.Unbox e, Num e, Eq e) => Sparse COO U e where 
    data instance SparseData COO U e   = COO { coo_vals :: U.Vector (e, Int, Int)
                                             , width :: Int
                                             , height :: Int
                                             }
...
transpose :: (U.Unbox e, Sparse COO ty e) => SparseData COO ty e -> SparseData COO D e
...
zipWith :: (Sparse COO ty a, Sparse COO ty1 b) => (a -> b -> c) -> SparseData COO ty a -> SparseData COO ty1 b -> SparseData COO D c
zipWith = SGeneric.zipWith


(#+) :: (Sparse COO ty a, Num a) => SparseData COO ty a -> SparseData COO ty a -> SparseData COO D a
(#+) = SGeneric.add 
...
scale :: (Sparse COO ty a, Num a) => a -> SparseData COO ty a -> SparseData COO D a 
