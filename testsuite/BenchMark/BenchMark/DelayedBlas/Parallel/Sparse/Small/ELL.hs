{-# LANGUAGE DataKinds #-}
module BenchMark.SparseBlas.Parallel.Sparse.Small.ELL where
...
import SparseBlas.Data.Matrix.Parallel.Sparse.ELL ( ELL ) 
...
bench_ell = do
   ...
   let 
      ...
      sp_data_m_ell    = M.map (\m -> manifest_convert m :: SparseData ELL U Double ) sp_data_m_coo 
   ...
   defaultMainWith config [
           bgroup "twice_atax_small_force_csc" [
              bench "tub100" $ nf to_vector 
                             $ ataxTwiceAddForce (fromJust $ M.lookup "tub100" sp_data_m_ell)
                                    (from_vector (fromJust $ M.lookup "tb" vec_data)) 
           ,  bench "pores_1_data" $ nf to_vector  
                                   $ ataxTwiceAddForce (fromJust $ M.lookup "pores_1_data" sp_data_m_ell) 
                                          (from_vector (fromJust $ M.lookup "pr1" vec_data)) 
           ,  bench "LF10_data" $ nf to_vector 
                                $ ataxTwiceAddForce (fromJust $ M.lookup "LF10_data" sp_data_m_ell)
                                       (from_vector (fromJust $ M.lookup "lf" vec_data)) 
           ,  bench "bcsstm03" $ nf to_vector 
                               $ ataxTwiceAddForce (fromJust $ M.lookup "bcsstm03" sp_data_m_ell)
                                      (from_vector (fromJust $ M.lookup "bcm" vec_data)) 
           ,  bench "bcsstk03" $ nf to_vector 
                               $ ataxTwiceAddForce (fromJust $ M.lookup "bcsstk03" sp_data_m_ell)
                                      (from_vector (fromJust $ M.lookup "bck" vec_data))
           ]

           ,   bgroup "twice_atax_small_no_force_csc" [
              bench "tub100" $ nf to_vector 
                             $ ataxTwiceAddNoForce (fromJust $ M.lookup "tub100" sp_data_m_ell)
                                    (from_vector (fromJust $ M.lookup "tb" vec_data)) 
           ,  bench "pores_1_data" $ nf to_vector  
                                   $ ataxTwiceAddNoForce (fromJust $ M.lookup "pores_1_data" sp_data_m_ell) 
                                          (from_vector (fromJust $ M.lookup "pr1" vec_data)) 
           ,  bench "LF10_data" $ nf to_vector 
                                $ ataxTwiceAddNoForce (fromJust $ M.lookup "LF10_data" sp_data_m_ell)
                                       (from_vector (fromJust $ M.lookup "lf" vec_data)) 
           ,  bench "bcsstm03" $ nf to_vector 
                               $ ataxTwiceAddNoForce (fromJust $ M.lookup "bcsstm03" sp_data_m_ell)
                                      (from_vector (fromJust $ M.lookup "bcm" vec_data)) 
           ,  bench "bcsstk03" $ nf to_vector 
                               $ ataxTwiceAddNoForce (fromJust $ M.lookup "bcsstk03" sp_data_m_ell)
                                      (from_vector (fromJust $ M.lookup "bck" vec_data))
           ]
