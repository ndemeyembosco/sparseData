{-# LANGUAGE DataKinds, BangPatterns, RankNTypes, ScopedTypeVariables #-}
...
--                -> (forall n1 n2. (KnownNat n1, KnownNat n2) => M.Map String (SparseData COO U n1 n2 Double))
...
--       sp_data_bigm_ell    = M.map (\m  -> let !m' = (manifest_convert m :: (forall n1 n2. (KnownNat n1, KnownNat n2) => SparseData COO U n1 n2 Double)) in m') sp_data_bigm_coo  
...
--                                     (fromJust $ from_vector (fromJust $ M.lookup "bcsstk09" big_vec_data)) 
--            ,  bench "bcsstk11" $ nf to_vector  
--                                    $ mvec (fromJust $ M.lookup "bcsstk11" sp_data_bigm_ell) 
--                                           (fromJust $ from_vector (fromJust $ M.lookup "bcsstk11" big_vec_data)) 
--            ,  bench "bcsstk14" $ nf to_vector 
--                                 $ mvec (fromJust $ M.lookup "bcsstk14" sp_data_bigm_ell)
--                                        (fromJust $ from_vector (fromJust $ M.lookup "bcsstk14" big_vec_data)) 
--         ]
--     ]
