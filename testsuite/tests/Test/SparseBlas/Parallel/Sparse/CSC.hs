...
instance (Arbitrary (SparseData O.COO U n1 n2 e), Undelay C.CSC n1 n2 e, Undelay O.COO n1 n2 e) 
         => Arbitrary (SparseData C.CSC U n1 n2 e) where 
             arbitrary = do 
                 (arr :: SparseData O.COO U n1 n2 e) <- arbitrary 
                 let (to_return :: SparseData C.CSC U n1 n2 e) = (manifest_convert arr) 
                 return to_return
...
test_csc = do 
    ...
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_convert_test :: SparseData C.CSC U 100 100 Double -> Bool)

    print " r (s A) == (r s) A: \n"
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_assoc_const_mul_prop :: Double 
                                    -> Double 
                                    -> SparseData C.CSC D 100 100 Double -> Bool)

    print "(A + B) == (B + A): \n"
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_commut_add_prop :: SparseData C.CSC D 100 100 Double 
                                        -> SparseData C.CSC D 100 100 Double -> Bool)

    print "r (A + B) = r A + r B: \n"
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_distr_const_mul_prop :: Double 
                                    -> SparseData C.CSC D 100 100 Double 
                                    -> SparseData C.CSC D 100 100 Double -> Bool)

    print "(A + B) + C == A + (B + C): \n"
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_assoc_add_prop :: SparseData C.CSC D 100 100 Double 
                                    -> SparseData C.CSC D 100 100 Double 
                                    -> SparseData C.CSC D 100 100 Double -> Bool)
    ...
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_scalar_vec_transform :: Int
                                    -> UVector.Vector Int 
                                    -> SparseData C.CSC D 100 100 Int -> Bool) 
    return ()