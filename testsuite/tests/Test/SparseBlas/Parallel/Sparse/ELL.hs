...
instance (Arbitrary (SparseData O.COO U n1 n2 e), Undelay E.ELL n1 n2 e) 
         => Arbitrary (SparseData E.ELL U n1 n2 e) where 
             arbitrary = do 
                 (arr :: SparseData O.COO U n1 n2 e) <- arbitrary 
                 let (to_return :: SparseData E.ELL U n1 n2 e) = (manifest_convert arr) 
                 return to_return
...
test_ell = do 
    ...
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_convert_test :: SparseData E.ELL U 100 100 Double -> Bool)

    print " r (s A) == (r s) A: \n"
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_assoc_const_mul_prop :: Double 
                                    -> Double 
                                    -> SparseData E.ELL D 100 100 Double -> Bool)

    print "(A + B) == (B + A): \n"
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_commut_add_prop :: SparseData E.ELL D 100 100 Double 
                                        -> SparseData E.ELL D 100 100 Double -> Bool)

    print "r (A + B) = r A + r B: \n"
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_distr_const_mul_prop :: Double 
                                    -> SparseData E.ELL D 100 100 Double 
                                    -> SparseData E.ELL D 100 100 Double -> Bool)

    print "(A + B) + C == A + (B + C): \n"
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_assoc_add_prop :: SparseData E.ELL D 100 100 Double 
                                    -> SparseData E.ELL D 100 100 Double 
                                    -> SparseData E.ELL D 100 100 Double -> Bool)
    ...
    quickCheckWith (stdArgs {maxSuccess=1000}) 
                    (s_scalar_vec_transform :: Int
                                    -> UVector.Vector Int 
                                    -> SparseData E.ELL D 100 100 Int -> Bool)
    return ()
